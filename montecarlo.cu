...
main( int argc, char* argv[ ] )
{
		...
		{
		hvs[n] = Ranf(VMIN, VMAX);    //cannon ball velocity
		hths[n] = Ranf(THMIN, THMAX); //cannonball angle
		hgs[n] = Ranf(GMIN, GMAX);    //ground to cliff
		hhs[n] = Ranf(HMIN, HMAX);    //height of cliff
		hds[n] = Ranf(DMIN, DMAX);    //distance to castle
		}
		...
		double megaTrialsPerSecond = (double)NUMTRIALS / (msecTotal / 1000.0) / 1000000.0;
		...
		float probability = (float)total_hits / (float)(NUMTRIALS);	// just get for last NUMTRIES run
		printf("time = %f ; Blocksize = %d ; trials = %8d  ; probability = %6.2f%% ; megatrials/sec = %f\n", msecTotal, BLOCKSIZE, NUMTRIALS, 100. * probability, megaTrialsPerSecond);
